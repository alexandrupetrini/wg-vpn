...
apt update
apt install wireguard wireguard-dkms wireguard-tools qrencode -y
...
if [ -z "$1" ]; then
  read -p "Enter the server address in the VPN subnet (CIDR format), [ENTER] set to default: 10.1.0.1: " SERVER_IP
  if [ -z $SERVER_IP ]; then
    SERVER_IP="10.1.0.1"
  fi
else
...
cat ./endpoint.var | sed -e "s/:/ /" | while read SERVER_EXTERNAL_IP SERVER_EXTERNAL_PORT; do
